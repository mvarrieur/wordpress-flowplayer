...
class Flowplayer5_Render {
	...
	public function player_render( $atts ) {
		if ( isset( $atts['playlist'] ) ) {
			...
			$atts = $this->playlist_single_video_config( $atts );
			return self::playlist_render( $atts );
		} else {
			$atts[ 'id' . $atts['id'] ] = self::get_shortcode_attr( $atts );
			$atts = self::single_video_config( $atts );
			return self::single_video_render( reset( $atts ) );
		}
	}
